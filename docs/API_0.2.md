...
{
- add __callForPermission__ step in LifeCyle 
- support promise in `enter`， `leave`, `canEnter` and `canLeave` to help us implement some asynchronous navigation.
- __remove some `global state in stateman`__ , all information about transition will be kept by transiton option.
- add namespace support for builtin emitter
%
