...
> {a little api in version 0.2 has been rewrited  % 微量的api在0.2版本有所修改}


# {Migrate from 0.1.x to 0.2.x %  从0.1版本迁移 }

{
- add [__callForPermission__](#permission) step in Lifecyle to stop navigation. 
- support return promise in `enter`， `leave` and `canEnter`, `canLeave`( introduced in v0.2.0) to help us implement some asynchronous navigation. 
- add [support](#namespace) for event with namespace
- Warn: __remove [state.async]__,  you can use  `option.async` for asynchronous navigation. but I suggest you to use promise to achieve the same function
%
- 增加了一个[callForPermission](), 来帮助我们组织一次跳转(比如在离开时， 加个通知用户是否要保存）
